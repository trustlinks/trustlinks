...
interface ReceivedReputationListProps {
  userPubkey: string;
}

function ReputationItem({ authorPubkey, rating, context, tag, content, createdAt, isPrivate }: {
  authorPubkey: string;
  rating: number;
  context?: string;
  tag?: string;
  ...
  const displayName = metadata?.name || metadata?.display_name || genUserName(authorPubkey);
...
export function ReceivedReputationList({ userPubkey }: ReceivedReputationListProps) {
  const { data: reputations, isLoading } = useReputation(userPubkey);
      ...
      <Card className="max-w-2xl mx-auto">
        <CardContent className="py-12 text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">
            Nie otrzymałeś jeszcze żadnej reputacji
          </p>
          <p className="text-sm text-gray-500 dark:text-gray-500 mt-2">
            Twoja reputacja pojawi się tutaj, gdy inni użytkownicy cię ocenią
          </p>
        </CardContent>
      </Card>
    );
    ...
    <div className="max-w-2xl mx-auto space-y-4">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Otrzymane reputacje ({reputations.length})
          </CardTitle>
        </CardHeader>
      </Card>
      ...
      {reputations
        ...
        .map((event) => {
          ...
          const isPrivate = event.kind === 4102;
